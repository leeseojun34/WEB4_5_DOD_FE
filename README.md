...
- 카카오톡 공유로 링크 전송
- 초대 링크에 `eventId`, `groupId` 파싱 → 참가자 자동 등록
- 그룹 일정의 경우 `group=true` 값으로 그룹 가입까지 처리
- 비로그인 사용자는 localStorage에 리디렉트 경로 저장 후 로그인 시 복귀

### 이벤트 페이지

- 이벤트 추가
  - 1회성 이벤트 추가 로직 구현
  - 그룹에서 이벤트 추가시 해당 로직으로 이벤트 생성
- 이벤트 조회
  - 생성한 이벤트에 대한 정보 열람 구현
- 나의 시간표 입력
  - 이벤트에서의 내가 가능한 시간대 입력 기능 구현
  - 드래그로 원하는 시간대 드래그 하여 입력 가능
  - XOR로 작업 되므로 드래그 후 입력 되고, 입력된 곳을 다시 드래그하여 영역을 선택하면 해제
- 내 시간표 불러오기
  - 마이페이지에서 저장한 즐겨찾는 시간대를 불러와서 해당 이벤트에 적용 가능
  - XOR로 작동하여 이미 체크된 부분의 경우 해제되면서 입력
- 일정 조율 결과 조회
  - 이벤트에 참여한 인원들의 참가 가능한 시간대를 입력을 완료후 결과 보기로 넘어가면
  - '가장 오래 만날 수 있는 시간', '가장 빨리 만날 수 있는 시간' 이렇게 두개의 카테고리로 3개 씩 추천 시간대 열람이 가능
- 이벤트 모든 인원 통합 시간표 표기
  - 이벤트에 초대된 인원들의 시간표 입력 현황을 표기
  - 겹치는 부분에는 진한 색으로 표기 하고 인원이 모자를수록 연해지는 방식으로 구현
- 일정 확정
  - 이벤트에 초대된 모든 인원의 조율결과 중 하나를 확정 짓는 기능 구현
- 즐겨찾는 시간 지정
  - 마이페이지에서 본인의 가능한 시간대를 입력할 수 있는 기능 구현
